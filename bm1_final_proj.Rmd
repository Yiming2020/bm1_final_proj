...
data set removes point9
```{r}

set.seed(2)
cv_df = 
  crossv_mc(crime_log_rm_9, 100)
...
cv_df = 
  ...
  mutate(
    mode_tran_nointer  = 
      map(.x = train, ~lm(ln_hate_crime_outcome ~ perc_population_with_high_school_degree + gini_index, data = .x)),
    mode_tran_inter  = 
      map(.x = train, ~lm(ln_hate_crime_outcome ~ perc_population_with_high_school_degree + gini_index*perc_non_white, data = .x)),
    mode_inter  = 
      map(.x = train, ~lm(hate_crimes_per_100k_splc ~ perc_population_with_high_school_degree + gini_index*perc_non_white, data = .x))
    ) %>% 
  mutate(
    rmse_tran_nointer = map2_dbl(.x = mode_tran_nointer, .y = test, ~rmse(model = .x, data = .y)),
    rmse_tran_inter = map2_dbl(.x = mode_tran_inter, .y = test, ~rmse(model = .x, data = .y)),
    rmse_inter = map2_dbl(.x = mode_inter, .y = test, ~rmse(model = .x, data = .y)))
...
data set doesn't remove point9
```{r}
set.seed(3)
cv_df = 
  crossv_mc(crime_log, 100)
...
reg_conf2 = lm(hate_crimes_per_100k_splc~perc_population_with_high_school_degree+gini_index + urbanization, data=crimes)
summary(model_fit)
summary(reg_conf2) ###  urbanization is  a confounder
...
reg_conf4 = lm(hate_crimes_per_100k_splc~perc_population_with_high_school_degree+gini_index + perc_non_citizen, data=crimes)
summary(model_fit)
summary(reg_conf4) # perc_non_citizen is a confounder
...
urbanization and perc_non_citizen  are confounders
...
final_fit_urhigh = lm(hate_crimes_per_100k_splc ~ perc_population_with_high_school_degree + gini_index,data=crime_urhigh)
summary(final_fit_urhigh)
par(mfrow=c(2,2))
plot(final_fit_urhigh) # need transformation

final_fit_urlow = lm(hate_crimes_per_100k_splc ~ perc_population_with_high_school_degree + gini_index,data=crime_urlow)
summary(final_fit_urlow)
par(mfrow=c(2,2))
plot(final_fit_urlow)
```
...
```{r}
boxcox(final_fit_urhigh)  # use natural log transformation
boxcox(final_fit_urlow) 
```
...
final_fit2_urhigh = 
  lm(ln_hate_crime_outcome ~ perc_population_with_high_school_degree + gini_index , data = crime_log_urhigh)
summary(final_fit2_urhigh)
##############################
crime_log_urlow = 
  crime_urlow %>%
  mutate(ln_hate_crime_outcome = log(hate_crimes_per_100k_splc, base = exp(1))) 
...
final_fit2_urlow = 
  lm(ln_hate_crime_outcome ~ perc_population_with_high_school_degree + gini_index , data = crime_log_urlow)
summary(final_fit2_urlow)
```

### Identify Collinearity
```{r}
# Calculate the variance inflation factor (VIF)
vif(final_fit2_urhigh)
vif(final_fit2_urlow)
```
no VIF higher than 5, nothing should be removed because of collinearity
...
influence.measures(final_fit2_urhigh)

# Look at the Cook's distance lines and notice obs 36 and 29 as potential Y outliers/influential points

par(mfrow=c(2,2))
plot(final_fit2_urhigh)


par(mfrow=c(2,2))
plot(final_fit2_urlow)
